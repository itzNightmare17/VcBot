...
async def play_music_(event):
    ...
    if not oreSongs.group_call.is_connected:
        if not (await oreSongs.vc_joiner()):
            return
        await oreSongs.group_call.start_audio(song)
...
async def radio_mirchi(e):
    ...
    oreSongs = Player(chat, e)
    if not oreSongs.group_call.is_connected and not (await oreSongs.vc_joiner()):
        return
    await oreSongs.group_call.start_audio(song)
    await xx.reply(
        f"• Started Radio 📻\n\n• Station : `{song}`",
        file="https://graph.org/file/65f90b37a6d97e236354c.jpg",
    )
    await xx.delete()
...
async def live_stream(e):
    ...
    await xx.delete()
    await oreSongs.group_call.start_audio(file)
